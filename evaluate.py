...
if __name__ == "__main__":
    ...
    for i in range(len(nn_txt)):
        for j in range(len(nn_txt[i])):
            if nn_txt[i][j] in ans_txt[i]:
                true_positives += 1
            else:
                false_positives += 1
    for i in range(len(ans_txt)):
        for j in range(len(ans_txt[i])):
            if ans_txt[i][j] not in nn_txt[i]:
                false_negatives += 1

    print('{}\'s result:'.format(para.nn))
    print('precision: {}'.format(cal_precision(true_positives, false_positives)))
    print('recall: {}'.format(cal_recall(true_positives, false_negatives)))
